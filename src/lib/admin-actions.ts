...
import { getEnvContext, getHeaders } from "@/lib/utils";
...
export async function upsertSection(
  ...
  try {
    ...
    return {
      success: false,
      error: e.message || "Failed to upsert section",
      status: 500,
    };
  }
}
...
export async function deleteSection(token: string, id: string) {
  try {
    ...
    await clearIncCache("/links");
    console.log(
      `Section with id ${id} deleted successfully. Revalidating path /links`,
    );
    return { success: true };
  } catch (e: any) {
    return {
      success: false,
      error: e.message || "Failed to delete section",
      status: 500,
    };
  }
}
...
export async function upsertLink(
  ...
  try {
    ...
    await clearIncCache("/links");
    console.log(
      `Link with id ${data.id} upserted successfully. Revalidating path /links`,
    );
    return { success: true };
  } catch (e: any) {
    return {
      success: false,
      error: e.message || "Failed to upsert link",
      status: 500,
    };
  }
}
...
export async function deleteLink(token: string, id: string) {
  try {
    ...
    await clearIncCache("/links");
    console.log(
      `Link with id ${id} deleted successfully. Revalidating path /links`,
    );
    return { success: true };
  } catch (e: any) {
    return {
...
export async function upsertSign(
  ...
  try {
    ...
    console.log(
      `Sign with id ${data.id} upserted successfully. Revalidating path /signs`,
    );
    return { success: true };
  } catch (e: any) {
    return {
      success: false,
      error: e.message || "Failed to upsert sign",
      status: 500,
    };
  }
}
...
export async function deleteSign(token: string, id: string) {
  try {
    ...
    console.log(
      `Sign with id ${id} deleted successfully.`,
    ...
    return {
      ...
      error:
        e.message ||
        "Failed to delete sign. Make sure to delete associated thoughts first.",
      status: 500,
    };
  }
}
...
// 4. THOUGHTS ACTIONS
// ==========================================
export async function upsertThought(
  token: string,
  data: {
    id: string;
    sign_id: string;
  ...
  try {
    ...
    return {
      success: false,
      error: e.message || "Failed to upsert thought",
      status: 500,
    };
  }
}
...
export async function deleteThought(token: string, id: string) {
  try {
    ...
    return {
      success: false,
      error: e.message || "Failed to delete thought",
      status: 500,
    };
  }
}
...
export async function uploadImageToR2(
  ...
  key: string,
  file: File,
): Promise<Partial<ReturnData<Record<"imageUrl", string>>>> {
...
export async function adminLogin(
  ...
  try {
    ...
    const isReqRateLmited = await isRateLimited(
      `ADMIN:LOGIN:${ipAddress}`,
      60,
      3,
    );
