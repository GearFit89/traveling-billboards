...
export async function setToken(token: string): Promise<ReturnStatus> {
  try {
    ...
    cookieStore.set(COOKIE_KEYS.ADMIN_COOKIE, signedToken, {
      maxAge: 60 * 60 * 24 * 7 * 6, //6 weeks
      httpOnly: true,
...
export async function isUsernameUnique(
  ...
  if (usernameCheckErr) {
    if(usernameCheckErr.message === 'duplicate key value violates unique constraint "profiles_username_key"'){
       throw new Error("Username is taken"); // If supa throws its unique row error, return a more friendly error to the ui
    }
...
export async function signUp({ username, email, password }: SignUpData) {
  try {
    ...
    const { error: profileErr } = await serverClient.from("profiles").insert({
      username,
      id: userId,
    });
